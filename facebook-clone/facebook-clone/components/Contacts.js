import Image from "next/image";
import React from "react";

const Contacts = ({ name, src, status }) => {
  return (
    <div className="flex items-center space-x-2 py-2 pl-1 hover:bg-gray-200 rounded-l-xl cursor-pointer relative">
      <Image
        // max-width={40}
        // max-height={40}
        rounded={40}
        src={src}
        height={40}
        width={40}
        className="rounded-full cursor-pointer max-w-10 max-h-10"
        alt="Joaquin Aman"
      />
      <p className="hidden sm:inline-flex text-sm font-semibold">{name}</p>
      {status === "online" ? (
        <div className="bg-green-800 h-3 w-3 rounded-full absolute left-5 bottom-2 border-2"></div>
      ) : (
        <div className="bg-gray-500 h-3 w-3 rounded-full absolute left-5 bottom-2 border-2"></div>
      )}

      {/* <div className="bg-green-900 h-4 w-4 rounded-full absolute left-5 bottom-2 border-2"></div> */}
    </div>
  );
};

export default Contacts;
